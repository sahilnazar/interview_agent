...
import { scheduleCandidate, sendInterviewerConfirmationRequest, sendScheduleRejectedEmail } from "../services/scheduler.js";
...
router.post("/:interviewerId/assign/:interviewId", requireAdmin, async (req, res, next) => {
  try {
    const { interviewerId, interviewId } = req.params;
    if (!isUuid(interviewerId) || !isUuid(interviewId)) {
      return res.status(400).render("error", { message: "Invalid interviewer or interview ID." });
    }

    // Check if this interviewer email is also a candidate for this interview
    const interviewerRow = await query("SELECT email FROM interviewers WHERE id = $1", [interviewerId]);
    if (!interviewerRow.rows.length) {
      return res.status(404).render("error", { message: "Interviewer not found." });
    }
    ...
    if (candidateRow.rows.length) {
      return res.status(409).render("error", { message: "This interviewer is already a candidate for this interview and cannot be assigned as interviewer." });
    }
...
router.post("/:interviewerId/unassign/:interviewId", requireAdmin, async (req, res, next) => {
  try {
    await query(
...
router.post("/schedule/:candidateId", requireAdmin, async (req, res, next) => {
  try {
    ...
    const cRow = await query("SELECT interview_id FROM candidates WHERE thread_id = $1", [candidateId]);
    if (!cRow.rows.length) return res.status(404).send("Candidate not found");

    const result = await scheduleCandidate(candidateId, cRow.rows[0].interview_id);
    if (!result.scheduled) {
      return res.status(409).send("No available slots found for any assigned interviewer");
    }
