...
router.post("/login/send-otp", async (req, res, next) => {
  try {
    ...
    if (!email) return res.render("interviewer-login", { step: "email", error: "Email is required", email: "" });

    const result = await query("SELECT * FROM interviewers WHERE email = $1", [email.trim().toLowerCase()]);
    if (!result.rows.length) {
      return res.render("interviewer-login", { step: "email", error: "No interviewer account found for this email", email });
    }

    await sendInterviewerOTP(result.rows[0]);
    req.session.otpEmail = email.trim().toLowerCase();
    res.render("interviewer-login", { step: "otp", error: null, email: email.trim().toLowerCase() });
  } catch (err) {
    next(err);
  }
});
...
router.post("/login/verify-otp", async (req, res, next) => {
  try {
    ...
    if (!interviewer) {
      return res.render("interviewer-login", { step: "otp", error: "Invalid or expired OTP", email });
    }

    delete req.session.otpEmail;
    req.session.interviewer = { id: interviewer.id, name: interviewer.name, email: interviewer.email };
...
router.post("/calendar/slots/:slotId/delete", requireInterviewer, async (req, res, next) => {
  try {
    await query(
...
router.post("/calendar/slots/bulk", requireInterviewer, async (req, res, next) => {
  try {
    const { from_date, to_date, days, slot_time_start, slot_time_end } = req.body;
    if (!from_date || !to_date || !slot_time_start || !slot_time_end) {
      return res.redirect("/interviewer/calendar");
    }

    const selectedDays = Array.isArray(days) ? days.map(Number) : days ? [Number(days)] : [];
    ...
    while (cursor <= end) {
      if (selectedDays.includes(cursor.getDay())) {
        const start = new Date(`${cursor.toISOString().slice(0, 10)}T${slot_time_start}:00`);
        const finish = new Date(`${cursor.toISOString().slice(0, 10)}T${slot_time_end}:00`);
        if (finish > start) {
          values.push([req.session.interviewer.id, start, finish, "available"]);
        }
      }
...
router.post("/schedule/:id/respond", requireInterviewer, async (req, res, next) => {
  try {
    ...
    if (decision === "confirm") {
      ...
      if (si.slot_id) {
        await query("UPDATE interviewer_slots SET status = 'booked' WHERE id = $1", [si.slot_id]);
      }
      // Send confirmation emails (handled by scheduling service)
      const { sendScheduleConfirmedEmails } = await import("../services/scheduler.js");
      ...
      if (si.slot_id) {
        await query("UPDATE interviewer_slots SET status = 'available' WHERE id = $1", [si.slot_id]);
      }
      const { sendScheduleRejectedEmail } = await import("../services/scheduler.js");
...
router.get("/confirm/:token", async (req, res, next) => {
  try {
    ...
    const si = result.rows[0];
    res.render("schedule-respond", { si, role: "interviewer", token, error: null });
  } catch (err) {
    next(err);
  }
});
  ...
  try {
    ...
    if (!["pending_interviewer", "pending_candidate"].includes(si.status)) {
      return res.render("schedule-respond", {
        si, role: "interviewer", token, error: "This slot has already been responded to."
      });
    }

    if (decision === "confirm") {
      await query("UPDATE scheduled_interviews SET status = 'confirmed' WHERE id = $1", [si.id]);
      if (si.slot_id) await query("UPDATE interviewer_slots SET status = 'booked' WHERE id = $1", [si.slot_id]);
      const { sendScheduleConfirmedEmails } = await import("../services/scheduler.js");
      await sendScheduleConfirmedEmails(si.id);
    } else {
      await query("UPDATE scheduled_interviews SET status = 'rejected_interviewer' WHERE id = $1", [si.id]);
      if (si.slot_id) await query("UPDATE interviewer_slots SET status = 'available' WHERE id = $1", [si.slot_id]);
      const { sendScheduleRejectedEmail } = await import("../services/scheduler.js");
